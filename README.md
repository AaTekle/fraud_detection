...
The parameter used in your implementation is:
...
| Symbol | Meaning |
|----------|----------|
| $d(A,B)$ | Actual distance between $A$ and $B$ |
| $k\text{-distance}(B)$ | Distance from $B$ to its $k$-th nearest neighbor |
| $\max(k\text{-distance}(B), d(A,B))$ | Selects the larger of the two distances |
| $reach\_dist_k(A,B)$ | Reachability distance between $A$ and $B$ |
